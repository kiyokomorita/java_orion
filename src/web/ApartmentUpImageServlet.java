...
/**
 * Servlet implementation class ApartmentUpImageServlet
 */
@WebServlet("/account/apartmentupimage")
@MultipartConfig(location="//PC3119/images2")
public class ApartmentUpImageServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (filename != null) {
			...
			//ApartmentImageフィールドにtrueを格納
			dto.setApartmentImage("true");
			request.setAttribute("dto", dto);
        ...
        switch(message){
            case "新規登録が完了しました":    // 物件検索ページに移動
                url = "/apartmentsearch";
                break;
            case "【エラー発生】入力内容を確認し、再度登録してください":    // 入力画面に移動
                url = "/account/apartment_add_input.jsp";
                break;
            default:    // 上記以外は確認画面に移動
                url = "/apartment_add_check.jsp";
                break;
        }
